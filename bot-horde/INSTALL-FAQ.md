...
`setup.sh` appends an `@bot-horde/BOTHORDE.md` reference to your `CLAUDE.md`. Claude Code loads that file into every session so the agent knows the Horde of Bots workflow.
...
No. Horde of Bots's files live under `bot-horde/` and at root (`setup.sh`). If your Pages source is `/docs`, Horde of Bots is invisible to Pages. If your Pages source is `/`, Horde of Bots is published alongside your site at `/bot-horde/`.
...
`bot-horde/MANIFEST.json` lists every file Horde of Bots manages, with a version date per file. Your local copy is what setup.sh installed. The upstream copy at `https://raw.githubusercontent.com/allavallc/bot-horde/main/bot-horde/MANIFEST.json` always reflects the current canonical versions.
...
**Can I run `curl … | bash`?**
Yes, with one caveat: if your repo has the *legacy* layout (root-level `BOTHORDE.md`, `build.sh`, etc.), the migration prompt needs a TTY. Without one, setup.sh detects no TTY and skips the migration with a log line. To migrate non-interactively: `HORDEOFBOTS_AUTO_MIGRATE=yes`.

**For CI use:**
```bash
curl -fsSL https://raw.githubusercontent.com/allavallc/bot-horde/main/setup.sh | HORDEOFBOTS_AUTO_MIGRATE=yes HORDEOFBOTS_UPGRADE_SKILL=yes bash
```
