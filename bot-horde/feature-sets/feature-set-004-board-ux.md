...
## Tickets
- HB-052 — Walking-robot perimeter animation on in-progress cards
- HB-053 — Animation demo, agent #1 (do not move)
- HB-054 — Animation demo, agent #2 (do not move)
- HB-055 — Animation demo, agent #3 (do not move)
- HB-056 — Animation demo, agent #4 (do not move)

## Status
Done — 2026-04-27

## Outcome
Robot animation shipped (HB-052). User approved the direction 2026-04-27 — per-agent SVG robot walks the card perimeter clockwise on a 12s loop, color hashed from `assigned_to`, random `animation-delay` per page load so multiple cards stagger naturally. Demo placeholders HB-053-056 served their visual-evaluation purpose and moved to `not-doing/`.
