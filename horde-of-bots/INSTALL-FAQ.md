...
`setup.sh` appends an `@horde-of-bots/HORDEOFBOTS.md` reference to your `CLAUDE.md`. Claude Code loads that file into every session so the agent knows the Horde of Bots workflow.

**What will the agent do autonomously?**
After you confirm a ticket draft (PM skill flow) or pick a ticket from the backlog, the agent will move the file between `horde-of-bots/` folders and `git add / commit / push` the move. It does **not** create tickets, change ticket bodies, or move tickets without your explicit confirmation. If you have stricter rules ("no autonomous git push"), state them in your own CLAUDE.md — the agent reads CLAUDE.md top-down so your rules override Horde of Bots's defaults.
...
**The block is wrapped in `<!-- Horde of Bots import block -->` markers.** Don't edit inside the markers; setup.sh manages that block. To disable Horde of Bots, remove the entire block (markers and all).
...
If your repo is public, yes — `horde-of-bots/*.md` files are world-readable on GitHub. There is no in-repo encryption.
...
If GitHub Pages is enabled on `main` / root, yes — the board auto-serves at `https://<owner>.github.io/<repo>/horde-of-bots/board.html`. Anyone with the URL can read it. Same caveat as the tickets: public repo = public board.

**To keep the board off Pages output:**
- Set Pages source to `/docs` (or any subfolder that doesn't include `horde-of-bots/`). The board still exists in your repo but isn't published.
- Or use a private repo + GitHub Pro for private Pages.

**Will Horde of Bots interfere with my existing Jekyll/Pages setup?**
No. Horde of Bots's files live under `horde-of-bots/` and at root (`setup.sh`). If your Pages source is `/docs`, Horde of Bots is invisible to Pages. If your Pages source is `/`, Horde of Bots is published alongside your site at `/horde-of-bots/`.
...
Fine-grained PAT, **Contents: Read and write**, scoped to the Horde of Bots repo only. That's the only scope used.
...
Whether the GitHub Actions workflow `git commit`s the rebuilt `horde-of-bots/board.html` back to `main` after a push. Default: `true`.

**When would I turn it off?**
If you don't want `chore: rebuild board [skip ci]` commits in your `git log`. Set `"auto_commit_board": false` in `horde-of-bots/config.json`. Trade-off: your Pages-hosted board goes stale until you rebuild manually (`bash horde-of-bots/build.sh && git add horde-of-bots/board.html && git commit -m "refresh board" && git push`).
...
Every 30 seconds by default. Override via `horde-of-bots/config.json` `poll_seconds` (minimum 10).
...
`~/.claude/skills/product-manager/SKILL.md`. Global to your machine, shared across every Horde of Bots project.

**How do I upgrade?**
Re-run `bash setup.sh`. If a newer version is upstream, you'll be prompted to overwrite. To skip the prompt: `HORDEOFBOTS_UPGRADE_SKILL=yes bash setup.sh`. To upgrade non-interactively: same env var + `< /dev/null`.
...
`horde-of-bots/MANIFEST.json` lists every file Horde of Bots manages, with a version date per file. Your local copy is what setup.sh installed. The upstream copy at `https://raw.githubusercontent.com/allavallc/horde-of-bots/main/horde-of-bots/MANIFEST.json` always reflects the current canonical versions.
...
Delete `horde-of-bots/HORDEOFBOTS.md` (or any file you want re-fetched) and re-run setup.sh.
...
**Can I run `curl … | bash`?**
Yes, with one caveat: if your repo has the *legacy* layout (root-level `HORDEOFBOTS.md`, `build.sh`, etc.), the migration prompt needs a TTY. Without one, setup.sh detects no TTY and skips the migration with a log line. To migrate non-interactively: `HORDEOFBOTS_AUTO_MIGRATE=yes`.

**For CI use:**
```bash
curl -fsSL https://raw.githubusercontent.com/allavallc/horde-of-bots/main/setup.sh | HORDEOFBOTS_AUTO_MIGRATE=yes HORDEOFBOTS_UPGRADE_SKILL=yes bash
```
...
Yes — `horde-of-bots/build.sh` uses Python 3 to parse tickets and render the board. Any of `py`, `python3`, or `python` (with version 3.x) works. The board itself runs in a browser; only the build step needs Python.
...
- Bugs: open an issue on https://github.com/allavallc/horde-of-bots
