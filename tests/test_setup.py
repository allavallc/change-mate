...
Local-only mode is signaled by `change-mate/` appearing in `.gitignore`. In that
mode `setup.sh` must NOT install `.github/workflows/change-mate-rebuild-board.yml`
(the workflow runs `bash change-mate/build.sh` in CI, which fails when
`change-mate/` is gitignored). Re-running setup.sh should also detect a stale
workflow and offer to remove it.

These tests pre-create every managed change-mate file in a tmp working dir so
...
WORKFLOW_REL = ".github/workflows/change-mate-rebuild-board.yml"


def _stage_repo(tmp_path: Path, gitignore_contents: str, with_workflow: bool = False) -> None:
    cm_dir = tmp_path / "change-mate"
    cm_dir.mkdir()
...
def test_local_only_mode_skips_workflow_install(tmp_path):
    ...
    _stage_repo(tmp_path, "change-mate/\n")
...
def test_local_only_mode_detection_ignores_unrelated_lines(tmp_path):
    """A .gitignore that mentions change-mate in a comment or a partial-match
    pattern (e.g. change-mate-output/, foo/change-mate/) must NOT trigger
    ...
    assert "local-only mode detected" not in result.stdout, \
        "false positive: .gitignore did not contain a true change-mate/ ignore rule"
