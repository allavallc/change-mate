...
Ensure the following folders exist inside `horde-of-bots/`, each with a `.gitkeep`:

```
horde-of-bots/backlog/.gitkeep
horde-of-bots/in-progress/.gitkeep
horde-of-bots/done/.gitkeep
horde-of-bots/blocked/.gitkeep
horde-of-bots/feature sets/.gitkeep
```
...
Create `horde-of-bots/feature sets/feature set-001.md`:
...
`build.sh` reads all ticket files and feature set files, then generates `horde-of-bots-board.html`.

The script should:

1. Read all `.md` files in `horde-of-bots/backlog/`, `in-progress/`, `done/`, `blocked/`
2. Parse each ticket file to extract: ID, title, status, priority, effort, assigned_to, started, completed
3. Read all `.md` files in `horde-of-bots/feature sets/`
4. Parse each feature set file to extract: name, goal, status, ticket IDs
5. Generate `horde-of-bots-board.html` with all data embedded as a JSON blob in a `<script>` tag
6. Print "horde-of-bots-board.html updated" when done

Make `build.sh` executable.

---

## Step 4 — Create horde-of-bots-board.html
...
## Step 5 — Update HORDEOFBOTS.md

Add the following rules to the existing `HORDEOFBOTS.md`:

### Feature Set rules

- A feature set is a named goal or milestone, not a time box
- Feature Set files live in `horde-of-bots/feature sets/`
...
- After every ticket is completed, run `bash build.sh` to regenerate `horde-of-bots-board.html`
- After every feature set is created or updated, run `bash build.sh`
- Never edit `horde-of-bots-board.html` directly — it is always generated by `build.sh`
...
- How to view the board (open `horde-of-bots-board.html` in a browser, or serve via GitHub Pages / Netlify / Vercel)
- How to regenerate the board manually: `bash build.sh`
- That `horde-of-bots-board.html` should be committed to the repo so teammates can always open the latest version

---

## Step 7 — Verify

1. Run `bash build.sh` — confirm `horde-of-bots-board.html` is generated
2. Open `horde-of-bots-board.html` in a browser — confirm both views load
...
- Do not put `horde-of-bots-board.html` in `.gitignore`
